...
inline void Render_DEBUG(float * __restrict outBuffer, size_t frames)
{
    ...
    const float32x4_t v_ab_mix = vdupq_n_f32((float32_t)getParameterValue(Parameters::ab_mix));
    const float32x4_t v_one_minus_ab_mix = vdupq_n_f32(1.0f - (float32_t)getParameterValue(Parameters::ab_mix));
        ...
        for (size_t i = 0; i < c_numVoices; ++i) {
            ...
            if (msample != 0.0f) {
                float32_t mallet_mix_vel = fmax(0.0f, fmin(1.0f,
                    (float32_t)getParameterValue(Parameters::mallet_mix) + 
                    voice.vel * (float32_t)getParameterValue(Parameters::vel_mallet_mix)));
            ...
            if (nsample != 0.0f) {
                float32_t noise_mix_vel = fmax(0.0f, fmin(1.0f,
                    (float32_t)getParameterValue(Parameters::noise_mix) + 
                    voice.vel * (float32_t)getParameterValue(Parameters::vel_noise_mix)));
...
void verifyAudioPath() {
    ...
    // Check 2: Gain
    float gain = (float)getParameterValue(Parameters::gain);
    ...
    printf("  Mallet mix: %.3f\n", getParameterValue(Parameters::mallet_mix));
    printf("  Noise mix: %.3f\n", getParameterValue(Parameters::noise_mix));
