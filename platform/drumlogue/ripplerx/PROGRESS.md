...
- Reviewed `6b57e12` refactor correctness:
  ...
  - voice/exciter reset paths centralized through constructors/reset helpers
  - no obvious state-init regressions found in the reviewed paths
...
- Implemented architectural **Step 2** (KS loss split): waveguide feedback now separates
  ...
  tuned independently.
- Implemented architectural **Step 3** (snare wire model): wire resonator Q is now
  velocity-controlled at NoteOn (hard hits -> tighter/high-Q crack, soft hits -> duller buzz).
- Implemented architectural **Step 4** (kick sub model): kick boom now sweeps
  approximately 90 -> 55 Hz using the decaying boom envelope as an exponential-like control.
...
## Phase 33: Stage-2 implementation pass (first 3 steps) [CURRENT]

Guiding references used:
- Sound On Sound Synth Secrets series on metallic percussion/cymbals/cowbells/snare
  and wind/brass synthesis (requested links).

Implemented steps:
1) **Metallic modal richness (CPU-aware)**
   - Extended modal pilot from 2 modes to up to 4 modes for metallic programs
     (`Cymbal`, `Gong`) while keeping 2 modes for baseline pilot programs.
   - Added NEON v7-compatible update path for modal recursion (modes 1/2 and 3/4).
   - Migrated modal oscillator coefficient init to `fastercosfullf` / `fastersinfullf`
     and decay conversion to `fasterexpf`.

2) **True dual-noise split (filtered low + unfiltered high)**
   - Preserved an unfiltered noise branch for the high burst while low-band remains
     filtered/LP-shaped.
   - This matches the intended snare/shaker/hat character split more faithfully.

3) **Kick sweep in semitone domain**
   - Replaced linear delay scaling sweep with exponential semitone-domain mapping
...
## Phase 34: Division-to-multiply micro-optimizations [CURRENT]

Optimization pass (requested):
- Replaced hot-path tone scaling divisions with precomputed reciprocal constants:
  - `tone / 10` -> `tone * 0.1`
  - `tone / 15` -> `tone * (1/15)`
- Added ARMv7-compatible NEON reciprocal path for Stage-4 soft clip
...
## Phase 54: Bullet-1 continuation — parallel HF exciter emphasis (May 16, 2026)

- Implemented Bullet-1 step 3 start for metallic families (`Cymbal`, `Gong`, `HHat-O`):
  - enforce higher high-band split (`noise_band_mix >= 0.92`)
  - reduce high-band rolloff (`noise_hi_lp_coeff >= 0.90`)
  - slightly lengthen high-band burst decay (`noise_env_hi.decay_rate *= 0.75` bounded)
- Goal: decouple metallic upper shimmer energy from the KS loop loss path and
  improve high-frequency persistence in cymbal/gong/open-hat attacks.

Bullet-1 status now
- Step 1: snare multiband wire rattle — implemented.
- Step 2: metallic low-loss loop clamps — implemented.
- Step 3: parallel HF exciter emphasis — implemented (initial lightweight pass).
- Next: evaluate score/descriptor deltas and decide whether a stronger dedicated
  multi-exciter branch is required for Cymbal/Gong.
