...
def compare_pair(ref_path: Path, ren_path: Path, auto_note_align: bool = False) -> Dict[str, object]:
    ...
    # Per-dimension scale normalization before cosine distance so large-Hz terms
    # do not dominate smaller descriptors (flatness, damping, entropy, ...).
    ref_vec_n = []
    ren_vec_n = []
    for a, b in zip(ref_vec, ren_vec):
        s = max(abs(a), abs(b), EPS)
        ref_vec_n.append(a / s)
        ren_vec_n.append(b / s)
    dot = sum(a * b for a, b in zip(ref_vec_n, ren_vec_n))
    na = math.sqrt(sum(a * a for a in ref_vec_n))
    nb = math.sqrt(sum(b * b for b in ren_vec_n))
