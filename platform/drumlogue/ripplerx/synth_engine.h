...
SynthState state;
    ...
    inline void LoadPreset(uint8_t idx) {
        ...
        static const int32_t presets[k_NumPrograms][k_lastParamIndex] = {
            ...
            {   3,  38,   0,   1, 120, 280,   0,   0,   2,   5,  98,  -3,   0,  46,   9,   4,   0,   2,   7,  71, 740,   2, 480, 707},        // 3:  Ac Snare   — brighter wire path: higher NzMix/NzRes/NzFq to feed the new stronger snare-wire resonator
            ...
            {   7,  41,   0,   1, 250, 390,   0,   0,   1,   5, 190,   4,   0,  30,  15,   4,   1,   7,   5,  14, 550,   0, 250, 707},       // 7:  Taiko      — harder mallet + reduced noise tail NzMx14 + lower NzFq for thud character
            {   8,  65,   0,   1, 720, 500,   0,   0,   1,   5, 170,  10,   0,  50,   8,  17,  25,  19,   5,  35, 800,   2, 620, 707},    // 8:  MrchSnr    — increase wire excitation brightness and reduce shell dominance
            ...
            {  12,  45,   0,   1, 450, 300,   0,   0,   2,   5, 120,  -2,   0,  44,  11,   3,   0,   7,   5,   8, 360,   0, 520, 707},       // 12: Ac Tom     — Drumhead gain curve: Dkay=90→g=0.697→T60≈195ms@110Hz; boom_mix=0.24 body lift
            {  13,  60,   0,   1, 800, 450,   0,   0,   0,   4, 200,  16,   0,   0,  18,   9,   5,  11,   5,  13, 640,   2, 340, 707},     // 13: Cymbal     — Dkay182/Mterl16/InHm7: balanced metallic with 6-mode bank + diffuser + noise bed
            {  14,  50,   0,   1, 200,  50,   0,   0,   0,   4, 200,   1,   0,   0,  20,   9,   1,  21,  20,  19, 860,   0,  30, 707},       // 14: Gong       — reduce darkness and bias more upper partial noise onset
            ...
            {  19,  84,   0,   1, 900, 500,   0,   0,   0,   1, 200,  14,   0,   0,  15,  20, 120,  20,   0,   5, 300,   0,1500, 707},  // 19: Triangle   — brighter loop settings and lower inharmonicity to preserve upper harmonics
            {  20,  36,   0,   1, 380, 350,   0,   0,   2,   5,  95,  -5,   0,  38,   6,   5,   0,   3,   6,  15, 220,   0, 220, 707},       // 20: Kick Drum  — MlSt350 (from 120): rendered 0-5ms had only 24% hi vs ref 82%; stiffer mallet sharpens beater click. Drumhead gain curve: Dkay=55→g=0.601→T60≈175ms@65Hz; boom_mix=0.40 dominates after body ring; pitch sweep 9st
            ...
            {  29,  79,   0,   1, 900, 490,   0,   0,   0,   4, 198,  16,   0,   0,  14,  13,   5,  12,   0,  79,1000,   2,1500, 707},  // 29: HHat-O  — brighter upper band and noise weighting
