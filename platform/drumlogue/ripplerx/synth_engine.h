...
inline static float preset_param(ProgramIndex program, ModelParamIndex param) {
	...
	// Tune boom to the sub-octave of the played note (harmonically related to the
        ...
        if (m_preset_idx == k_Taiko) {
            ...
            v.boom_env        = 1.0f;
            v.boom_decay      = preset_param(k_KickDrum, k_boom_decay); // 0.99940
            v.boom_mix        = preset_param(k_KickDrum, k_boom_mix);   // 0.40
            v.boom_attack_env = 0.0f;
            v.boom_attack_inc = preset_param(k_KickDrum, k_boom_attack_inc); // 0.0010
        } else if (m_preset_idx == k_AcousticTom) {
            // AcTom: subtle 110 Hz low-body reinforcement after the membrane ring.
            v.boom_inc        = tom_bm;
            v.boom_env        = 1.0f;
            v.boom_decay      = preset_param(k_AcousticTom, k_boom_decay); // 0.99945
            v.boom_mix        = preset_param(k_AcousticTom, k_boom_mix);   // 0.05
            v.boom_attack_env = 0.0f;
            v.boom_attack_inc = preset_param(k_AcousticTom, k_boom_attack_inc); // 0.0008
        }
