...
fast_inline void metal_engine_update(metal_engine_t* metal,
    ...
    for (int i = 0; i < 4; i++) {
        // prefer something less harmonic to avoid organ-like or buzzing sound
        // different per operator
        float32x4_t rand = neon_generate_float_rand_0_1(&metal->noise_prng);
        // OLD ratio = 1.0 + (base_ratio - 1.0) * inharmonicity * spread
        // float32x4_t ratio_offset = vsubq_f32(metal->base_ratio[i], one);
        float32x4_t ratio_offset = vsubq_f32(metal->base_ratio[i], rand);
        float32x4_t spread_factor = vmulq_f32(metal->inharmonicity, spread_range);
        metal->current_ratio[i] = vaddq_f32(rand,
...
fast_inline float32x4_t metal_engine_process(metal_engine_t* metal,
    ...
    // phase randomization per trigger
    float32x4_t rand = neon_generate_float_rand_0_1(&metal->noise_prng);
    float32x4_t rand_phase = vmulq_f32(rand,
                                       vdupq_n_f32(2.0f * M_PI));
    // 2. Advance all 4 phases
    for (int i = 0; i < NEON_LANES; i++) {
        float32x4_t freq = vmulq_f32(base_freq, metal->current_ratio[i]);

        float32x4_t jitter = neon_generate_float_rand_tiny(&metal->noise_prng);
        jitter = vaddq_f32(jitter, one);
        freq = vmulq_f32(freq, jitter);

        metal->phase[i] = vbslq_f32(active_mask, rand_phase, metal->phase[i]);
    ...
    // 2.1 create a subtle instability to reduce simmetry add some realism
    float32x4_t jitter = neon_generate_float_rand_tiny(&metal->noise_prng);
    jitter = vaddq_f32(jitter, one);
    ...
    // index_i = base_index * (1.0f + jitter);
    // This alone reduces “synthetic buzz” a lot.
    jitter = vaddq_f32(rand, one);  // rand [0, 1)
    base_index = vmulq_f32(base_index, jitter);
    float32x4_t index_high = vmulq_n_f32(base_index, 3.0f);

    // OLD: 4→3→2→1
    // Op4 -> Modulates Op2 (Dies very fast — genv4; gate further shortens this)
    float32x4_t op4 = neon_sin(metal->phase[3]);
    float32x4_t phase2_mod = vaddq_f32(metal->phase[1], vmulq_f32(op4, vmulq_f32(index_high, genv4)));
    float32x4_t op2 = neon_sin(phase2_mod);

    // (4 → 2) + (3 → 1) for less correlation
    // float32x4_t phase2_mod = vaddq_f32(metal->phase[1], vmulq_f32(op3, vmulq_f32(base_index, genv2)));
    float32x4_t op3 = neon_sin(metal->phase[2]);
    // Op3 -> Modulates Op1 (Sustains — gated_env; gate brings it down over time)
    float32x4_t phase1_mod = vaddq_f32(metal->phase[0], vmulq_f32(op3, vmulq_f32(base_index, gated_env)));
    ...
    float32x4_t fm_output = vmulq_f32(vaddq_f32(vaddq_f32(op1, op3), harmonics), gated_env);
    ...
    {
      float32x4_t white = neon_generate_float_white_noise(&metal->noise_prng);
      ...
      // add some stick attack and “air burst”, removes sterile feel
      float32x4_t env_fast = vmulq_f32(envelope, envelope);  // or env^3
      float32x4_t noise_transient = vmulq_f32(noise_hp, env_fast * 0.4f);
      fm_output = vaddq_f32(fm_output, noise_transient);
    }
