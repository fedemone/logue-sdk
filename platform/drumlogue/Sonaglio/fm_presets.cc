...
const fm_preset_t FM_PRESETS[NUM_OF_PRESETS] = {
    // Preset 0: "Deep Tribal" (original)
    {
        ...
        0, 80, 60, 30,
        70, 40, 50, 50,
        40, 0, 20, LFO_TARGET_PITCH, 30,
        0, 5, LFO_TARGET_INDEX, 20,   // EuclTun=0 (Off)
        20, 0,  // env_shape, voice_in2, 50, 30,
    },
    ...
    // Preset 3: "Phase Dance" (original)
    {
        ...
        0, 30, 40, 50,
        60, 50, 60, 40,
        30, 3, 45, LFO_TARGET_LFO2_PHASE, 70,
    ...
    // Because LFO phase resets on each trigger, this is one-shot per hit.
    {
        ...
        0, 70, 60, 20,
        40, 0, 0, 0,
        0, 1, 5, LFO_TARGET_ENV, 60,      // LFO1: 0.5 Hz ramp → ENV swell
    ...
    // gives a fast wah on every hit; synced to trigger so it starts fresh.
    {
        ...
        0, 60, 50, 30,
        40, 0, 0, 60,
        50, 0, 35, LFO_TARGET_RES_MORPH, 80,  // LFO1: triangle 4 Hz → filter morph
    ...
    // patterns: each snare hit starts with crack, fades to hiss.
    {
        ...
        0, 30, 50, 50,
        60, 50, 60, 0,
        0, 1, 5, LFO_TARGET_NOISE_MIX, 90,  // LFO1: slow ramp → noise blend
    ...
    // The difference frequency (5 Hz) causes a slow tremolo on top.
    {
        ...
        0, 50, 40, 40,
        60, 70, 80, 50,
        50, 0, 82, LFO_TARGET_INDEX, 70,   // LFO1: ~15 Hz triangle → FM index buzz
    ...
    // different tonal colour.  Voice alloc K-S-M-R.
    {
        ...
        0, 40, 40, 70,
        50, 40, 50, 0,
        0, 0, 18, LFO_TARGET_RES_MORPH, 60,  // LFO1: 2 Hz triangle → filter colour
    ...
    // for a single hit since LFO resets on trigger).
    {
        ...
        0, 20, 30, 40,
        50, 60, 70, 50,
        40, 1, 25, LFO_TARGET_PITCH, -70,  // LFO1: ramp, negative → falling pitch
        0, 0, LFO_TARGET_NONE, 0,
        10, 70, 40, 60,
    },
    ...
    // back, giving a pitch/wah character.  Voice alloc K-S-M-R.
    {
        ...
        0, 50, 60, 0,
        0, 0, 0, 60,
        60, 0, 10, LFO_TARGET_RES_MORPH, 100, // LFO1: 1 Hz triangle → full morph sweep
    ...
    // near-audio LFO on NOISE_MIX for textured shimmer (metal and snare).
    {
        ...
        0, 20, 40, 80,
        70, 80, 90, 60,
        70, 0, 75, LFO_TARGET_NOISE_MIX, 80,  // LFO1: ~10 Hz triangle → noise shimmer
    ...
    // rich gong spectrum, slow LFO pitch sag (gong pitch drops after strike).
    {
        ...
        0, 0, 0, 0,
        0, 70, 60, 0,
        0, 1, 8,  LFO_TARGET_PITCH, -40,  // LFO1: slow ramp → pitch sag after hit
        0, 0,  LFO_TARGET_NONE, 0,
        128, 0, 0, 0                  // EnvShape: char=1(Gong) + env=110 (long dec0, 0, 0,
    },
    ...
    // attack transient then softens.  Long decay envelope.
    {
        ...
        0, 0, 0, 0,
         0, 20, 80, 0,
         0, 0, 12, LFO_TARGET_INDEX, 60,   // LFO1: slow ramp → brightness decay
        0, 0,  LFO_TARGET_NONE, 0,
        128, 0, 0, 0                  // EnvShape: char=1(Gong) + env=100 (lo0, 0, 0,
    },
    ...
    // texture.  Medium decay, mid-range voices active.
    {
        ...
        0, 50, 40, 20,
        30, 60, 70, 0,
        0, 0, 40, LFO_TARGET_NOISE_MIX, 70, // LFO1: triangle → noise texture
        0, 6,  LFO_TARGET_PITCH, -20,     // LFO2: slow pitch drop; EuclTun=0
        128, 0, 0, 0                    // EnvShape: char=1(Gong) + env=60 (medi0, 0, 0,
    },
    ...
    // percussion.  Slow ramp LFO→Pitch adds a falling pitch tail to each hit.
    {
        ...
        0, 60, 50, 25,
        55, 50, 65, 55,
        35, 1, 12, LFO_TARGET_PITCH, -30,  // LFO1: slow ramp → falling pitch tail
    ...
    // tuned perc and metal voices dominate.  Short env for staccato character.
    {
        ...
        0, 30, 40, 15,
        35, 35, 55, 70,
        45, 1, 18, LFO_TARGET_PITCH, -20,  // LFO1: ramp → slight pitch drop after hit
    ...
    {
        "HiHatSw", 70, 50, 100, 10,
        0, 25, 45, 40,
        45, 55, 90, 0,
        0, 1, 30, LFO_TARGET_METAL_GATE, 90,  // LFO1: Ramp → open→closed gate
