...
ENGINE_MAPPING_INLINE engine_macro_targets_t mapping_compute_engine(engine_id_t engine,
  ...
  switch (engine) {
    case ENGINE_KICK:
      return mapping_compute_kick(p1, p2, g, velocity);
    case ENGINE_SNARE:
      return mapping_compute_snare(p1, p2, g, velocity);
    case ENGINE_METAL:
      return mapping_compute_metal(p1, p2, g, velocity);
    case ENGINE_PERC:
      return mapping_compute_perc(p1, p2, g, velocity);
    default:
      return mapping_zero_targets();
  }
}
...
fast_inline float32x4_t fm_soft_clip(float32x4_t x) {
    float32x4_t den = vaddq_f32(vdupq_n_f32(1.0f), vabsq_f32(x));
    float32x4_t recip = vrecpeq_f32(den);
    // Newton-Raphson refinement: recip = recip * (2.0 - den * recip)
