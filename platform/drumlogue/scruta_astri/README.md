...
| Filter 2 always self-oscillating | The Euler-forward guard in `PolivoksFilter::set_coeffs` used the linear condition `f² + 2fq < 4`. Both of that filter's integrators and its resonance feedback run through `fast_tanh`, which has a small-signal gain of 1.5, so the real condition is `1.5f² + 3fq < 4` — the guard was 2.25× too loose. Fed literal silence the filter still put out ~0.45 forever, and since it sits downstream of both oscillators, nothing upstream could ever make the unit quiet | Guard corrected to `f_max = √(q² + 8/3) − q`, which predicts every measured case. Self-oscillation is now [opt-in on `F2Res`](#filter-2-self-oscillation), amplitude-limited rather than clamp-limited |
...
`F2Res` above **67** hands filter 2 over to self-oscillation. It takes off around 72 and gets louder from there; the whole top third of the knob is the ramp. Below 67 the filter is provably stable and decays to actual silence on a silent input.

The mechanism is van der Pol style: the resonance feedback is negative while the filter is quiet, so the oscillation grows, and climbs back through zero as the state grows, so it settles at a fixed amplitude. Simply inverting the damping does not work — `fast_tanh` saturates the feedback at a constant instead of reducing it, so the states ramp until something else stops them.

This matters beyond the noise it makes: filter 2 is downstream of both oscillators, so while it sings, nothing upstream of it can make the unit quiet. Every gate, strike and add-on in this document depends on it being able to stop.
...
Measured at the bus on preset 12 with Osc 2 muted, the strike gates to full silence at every cutoff and resonance — as long as `F2Res` stays under the [howl threshold](#filter-2-self-oscillation). Crank it past 67 and filter 2 sings through the gaps, which is a choice rather than the filter's resting state.
