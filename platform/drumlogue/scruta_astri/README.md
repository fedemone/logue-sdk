...
* **The Morphing Filter:** The `CMOSDist` parameter morphs Filter 1 smoothly from a Clean SVF (0-33) -> Moog-style symmetrical saturation (34-65) -> Sherman Filterbank asymmetrical chaos (66-100). The three zones use different machinery: the middle one saturates the integrators, the top one switches them back to linear and puts an asymmetrical wavefolder in front instead. Its screaming is on demand too, from the top third of `F1Res` — see [Filter Self-Oscillation](#filter-self-oscillation).
  * The wavefolder costs headroom: it injects DC by design (`drive_sig += sherman_asym`), and although the output DC blocker takes most of it out, the bus sits at about −15 dBFS of DC at `CMOS` 100 against −19 below the zone.
...
| Sherman zone inert | `CMOSDist` 67-100 is documented as asymmetrical wavefolding, but its asymmetry was computed as `(value − 66) / (percent_normalizer − 66) × 2` — and `percent_normalizer` is `0.01`, where `100` was meant. The denominator was −65.99, so the whole zone came out negative, `filter1.sherman_asym` clamped it to 0 and the wavefolder never ran; the makeup gain derived from it fell to 0.485 instead of rising to 2.0. Measured across 67-100, the knob moved RMS by 0.25 dB and added no harmonics at all | Denominator corrected to `100 − 66`. Asymmetry now ramps 0→2 and makeup 1→2 across the zone. Zero crossings go from 829/s at `CMOS` 66 to 1756 at 85 and 2302 at 100, and crest factor recovers from a square-wave 1.2 dB to 5.6-6.4 dB. `CMOS` 0-66 is bit-identical |
