...
typedef struct {
    ...
    float am_level;   /* = volume * 0.5, used by amplitude-modulation algos   */
    float am_offset;  /* = 1 - am_level                                       */
...
static inline void fmo_set_volume(fm_op_t* op, float v) {
    ...
    op->am_level  = op->volume * 0.5f;
    op->am_offset = 1.0f - op->am_level;
}
...
/* Amplitude modulator: returns a positive 0..1 gain (am_offset + am_level*wave)
 * used by the *amp* algorithms (14..17) to scale a carrier's envelope. */
static inline float fmo_am(fm_op_t* op, float mod_in, float pitch) {
    float t = fmo_advance(op, mod_in, pitch);
    float s = fmo_wf_render(op->waveform, t);
    op->last_out = s;
    return op->am_offset + op->am_level * s;
}
